...
LMatrix::LMatrix(int _n, int _m) : n(_n), m(_m)
{
	if (n == 0 || m == 0) throw "Matix size is 0\n";	
	data.resize(n);
	for (auto& i : data) i.resize(m);
}

void LMatrix::init(int _n, int _m) 
{
...
//read from stream
void LMatrix::read(std::ifstream& stream) {
	for (int i = 0; i < n; i++)
		for (int j = 0; j < m; j++)
		{
			stream >> data[i][j];
